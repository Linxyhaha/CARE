...
def parse_global_args(parser):
    ...
    parser.add_argument("--base_model", type=str, default="google-t5/t5-small",help="basic model path")
    ...
    # reasoning
    parser.add_argument("--progressive_attn", action="store_true", help="whether or not enable adpative attention for differenet stage of reasoning")
    parser.add_argument("--attention_strategy", default="hard", help="attention strategy used in adpative attention, e.g., hard")

    # indepth (for customized query number and progressive attention)
    parser.add_argument("--query_list", type=int, nargs="+", help="list of reasoning steps per stage, e.g., --query_list 1 1 1 1")
    parser.add_argument("--progressive_list", type=int, nargs="+", help="list of whether or not use adpative attention per stage, e.g., --query_list 1 1 1 0")
    
    # diversity
    parser.add_argument("--query_div_scale", type=float, default=0.1, help="strength of query diversity loss")
...
def parse_train_args(parser):
    ...
    parser.add_argument("--resume_dont_load_training_state", action="store_true")
    ...
    parser.add_argument("--valid_sample", type=int, default=-1)

    parser.add_argument("--wandb_project", default="TIGER-qwen", type=str)
    parser.add_argument("--wandb_run_name", default=None, type=str)
    parser.add_argument("--wandb_watch", default="all", type=str)

    # llm param
    parser.add_argument("--llm_param", default="open", type=str, help="whether setting the llm params learnable or frozen") 
    
    return parser

def parse_test_args(parser):

    parser.add_argument("--ckpt_path", type=str, default="/./ckpt/games/", help="The checkpoint path")
...
def load_datasets(args):
    ...
    for task in tasks:
        if task.lower() == "seqrec":
            dataset = SeqRecDatasetCSV(args, mode="train")
        else:
            raise NotImplementedError
        train_datasets.append(dataset)
    train_data = ConcatDataset(train_datasets)
    valid_data = SeqRecDatasetCSV(args,"valid")
...
def load_test_warm_dataset(args):
    if args.test_task.lower() == "seqrec":
        test_data = SeqRecDatasetCSV(args, mode="test_warm", sample_num=args.sample_num)
    else:
        raise NotImplementedError
    return test_data
...
def load_test_cold_dataset(args):
    if args.test_task.lower() == "seqrec":
        test_data = SeqRecDatasetCSV(args, mode="test_cold", sample_num=args.sample_num)
    else:
        raise NotImplementedError
    return test_data
